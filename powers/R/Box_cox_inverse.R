#' Box-Cox Transform Inverse Function
#'
...
#'
#' @return A vector with a transformed value. It returns \code{exp(x)} for lambda equals 0, and \code{(lambda*x + 1)^(1/lambda)} for lambda non-zero.


#' @export
Box_cox_inverse <- function(x, lambda = 0, change_neg_to_pos=0) {
    # Checking for errors
    if((change_neg_to_pos < 0)) stop("Please enter a non-negative change_neg_to_pos value.")

    # makepos makes each x positive.
    if(any(x < 0)){
    ...
    # Computation of the inverse of Box-Cox Function
